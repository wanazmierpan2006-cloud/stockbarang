...
<div class="space-y-6" x-data="{ selectedTx: null }">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white">Laporan Barang Keluar</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Rekapitulasi pengeluaran barang ke divisi atau unit penerima dengan filter data lengkap.</p>
        </div>
        
        <div class="flex items-center space-x-2">
            <a href="{{ route('reports.export-pdf', array_merge(['type' => 'outgoing'], request()->all())) }}" target="_blank"
               class="px-4 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-2xl text-xs font-extrabold shadow-md shadow-rose-600/20 transition flex items-center space-x-2">
                <i class="fa-solid fa-print"></i>
                <span>Cetak / Export PDF</span>
            </a>
            
            <a href="{{ route('reports.export-excel', array_merge(['type' => 'outgoing'], request()->all())) }}"
    ...
    <div class="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <form method="GET" action="{{ route('reports.outgoing') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end text-xs">
            <div>
                <label class="block font-bold text-slate-600 dark:text-slate-300 mb-1">Tanggal Mulai</label>
                <input type="date" name="start_date" value="{{ $startDate }}" class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:border-rose-500">
            </div>
            
            <div>
                <label class="block font-bold text-slate-600 dark:text-slate-300 mb-1">Tanggal Sampai</label>
                <input type="date" name="end_date" value="{{ $endDate }}" class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:border-rose-500">
            </div>

            <div>
                <label class="block font-bold text-slate-600 dark:text-slate-300 mb-1">Bulan</label>
                <select name="month" class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:border-rose-500">
            ...
            <div>
                ...
                <select name="year" class="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:border-rose-500">
            ...
            <div class="flex items-center space-x-2">
                <button type="submit" class="w-full py-2.5 bg-rose-600 hover:bg-rose-700 text-white font-extrabold rounded-xl transition">
                    <i class="fa-solid fa-filter mr-1"></i> Filter
                </button>
                <a href="{{ route('reports.outgoing') }}" class="px-3 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold rounded-xl hover:bg-slate-200 transition" title="Reset Filter">
            ...
            <table class="w-full text-left text-xs">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition">
                        ...
                        <td class="px-6 py-4 font-mono font-bold text-rose-600 dark:text-rose-400">{{ $row->kode_transaksi }}</td>
                        <td class="px-6 py-4 font-mono text-slate-600 dark:text-slate-300">{{ $row->tanggal->format('d/m/Y') }}</td>
                        <td class="px-6 py-4 font-bold text-slate-900 dark:text-white">{{ $row->tujuan }}</td>
                        ...
                        <td class="px-6 py-4 text-center">
                            <button type="button" @click="selectedTx = {{ json_encode($row->load(['user', 'details.item'])) }}" 
                                    class="p-2 text-slate-400 hover:text-rose-600 rounded-xl hover:bg-rose-50 transition" title="Lihat Detail Item">
                    ...
                    @empty
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center text-slate-400">Tidak ada data transaksi barang keluar sesuai filter.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl max-w-2xl w-full p-6 sm:p-8 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-5" @click.outside="selectedTx = null">
            ...
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
                <div>
                    ...
                    <h3 class="font-extrabold text-slate-900 dark:text-white text-lg mt-1">Rincian Barang Keluar</h3>
                </div>
